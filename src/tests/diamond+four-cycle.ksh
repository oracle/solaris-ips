...
# publish six packages with a dependency graph:
#
#       F
#      / \ 
#     D<--E
...
eval `pkgsend open test/diamond4cycle/A@0.1-1`
if [ $? != 0 ]; then
	echo \*\* script aborted:  couldn\'t open test/diamond4cycle/A
	exit 1
fi
...
if [ $? != 0 ]; then
	echo \*\* script aborted:  couldn\'t open test/diamond4cycle/libB
	exit 1
fi
...
if [ $? != 0 ]; then
	echo \*\* script aborted:  couldn\'t open test/diamond4cycle/libC
	exit 1
fi
...
if [ $? != 0 ]; then
	echo \*\* script aborted:  couldn\'t open test/diamond4cycle/libD
	exit 1
fi
...
if [ $? != 0 ]; then
	echo \*\* script aborted:  couldn\'t open test/diamond4cycle/libE
	exit 1
fi
...
if [ $? != 0 ]; then
	echo \*\* script aborted:  couldn\'t open test/diamond4cycle/libF
	exit 1
fi
