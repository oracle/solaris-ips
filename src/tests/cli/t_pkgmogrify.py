...
"""<transform file -> add bobcat 1>"""
        ...
        def setUp(self):
                ...
                for i, s in enumerate(self.transforms):
                        fname = os.path.join(self.__test_dir, "transform_%s" % i)
        ...
        def test_1(self):
                ...
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s | egrep -v SUNWxorg-mesa" % source_file)
                self.pkgmogrify("-Di386_ONLY=' ' -DBUILDID=0.126 %s | egrep SUNWxorg-mesa@7.4.4-0.126" % source_file)
                # nested macros
                self.pkgmogrify("-Di386_ONLY=' ' -DBUILDID='$(FOO)' -DFOO=0.126 %s | egrep SUNWxorg-mesa@7.4.4-0.126" % source_file)

        def test_2(self):
                """display output to files """
                source_file = os.path.join(self.__test_dir, "source_file")
                output_file = os.path.join(self.__test_dir, "output_file")
                self.pkgmogrify("-Di386_ONLY=' ' -DBUILDID=0.126 -O %s %s ; egrep SUNWxorg-mesa@7.4.4-0.126 %s" % (
        ...
        def test_3(self):
                ...
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s %s | egrep -v X11" % (self.transform_files[1], source_file))
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s %s | egrep Y11" % (self.transform_files[1], source_file))
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s %s | egrep bobcat | wc | egrep '3'" % (self.transform_files[10], source_file))                
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s %s | egrep -v mode=0755" % (self.transform_files[4], source_file))
 
        def test_4(self):
                source_file = os.path.join(self.__test_dir, "source_file")
                output_file = os.path.join(self.__test_dir, "output_file")
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s %s | egrep -v xkbprint" % (self.transform_files[2], source_file))
        ...
        def test_5(self):
                ...
                self.pkgmogrify(("-Di386_ONLY='#' -DBUILDID=0.126 %s %s |" + 
                    "egrep 'file NOHASH group=bin mode=0555 owner=root path=usr\/X11\/bin\/Xserver'") % (self.transform_files[3], 
                    source_file))
                self.pkgmogrify(("-Di386_ONLY='#' -DBUILDID=0.126 %s %s %s|" + 
                    "egrep 'file NOHASH group=bin mode=0755 owner=root path=usr\/Y11\/bin\/Xserver'") %  \
                    (self.transform_files[1], self.transform_files[3], source_file))
                self.pkgmogrify(("-Di386_ONLY='#' -DBUILDID=0.126 -I %s %s %s |" + 
                    "egrep 'file NOHASH group=bin mode=0755 owner=root path=usr\/Y11\/bin\/Xserver'") %  \
                    (self.__test_dir, self.transform_files[7], source_file))
               # check multiple modes to the same attribute on same action
                self.pkgmogrify(("-Di386_ONLY='#' -DBUILDID=0.126 %s %s %s |" + 
                    "egrep 'file NOHASH group=bin mode=0555 owner=root path=usr\/Y11\/bin\/Xserver'") % \
                    (self.transform_files[3], self.transform_files[1], source_file))                                    
        ...
        def test_6(self):
                ...
                self.pkgmogrify(("-Di386_ONLY='#' -DBUILDID=0.126 %s %s %s |" + 
                    "egrep 'file NOHASH group=bin mode=0555 owner=root path=usr\/Y11\/bin\/bdftopcf") % \
                    (self.transform_files[3], self.transform_files[1], source_file))
        ...
        def test_7(self):
                ...
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 --froob", exit=2)
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s" % self.transform_files[8], exit=1) # file not found
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 -I %s %s" % (self.__test_dir, self.transform_files[8]), exit=1) # nested tranform error
                self.pkgmogrify("-Di386_ONLY='#' -DBUILDID=0.126 %s %s" % (self.transform_files[6], source_file), exit=1) # bad transform
